...
import { adminListRelationshipsFn, adminUpsertRelationshipFn, adminDeleteRelationshipFn, type TaxonomyItem } from "@/lib/taxonomy.functions";
...
export const Route = createFileRoute("/admin/relationships")({ component: AdminRelationships });
...
function AdminRelationships() {
  ...
  const { data: items = [] } = useQuery({ queryKey: ["admin-relationships"], queryFn: () => adminListRelationshipsFn() });

  const save = useMutation({
    mutationFn: () => adminUpsertRelationshipFn({ data: {
      ...form,
    ...
    onSuccess: (r) => { if (!r.ok) return toast.error(r.error); toast.success("Saved"); setOpen(false); setForm(empty); qc.invalidateQueries({ queryKey: ["admin-relationships"] }); },
  });
  const del = useMutation({
    mutationFn: (id: string) => adminDeleteRelationshipFn({ data: { id } }),
    onSuccess: () => { toast.success("Deleted"); qc.invalidateQueries({ queryKey: ["admin-relationships"] }); },
  });
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h1 className="font-display text-2xl font-bold">Relationships ({items.length})</h1>
        <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) setForm(empty); }}>
          <DialogTrigger asChild><Button><Plus className="size-4 mr-1.5" /> New relationship</Button></DialogTrigger>
          <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
            <DialogHeader><DialogTitle>{form.id ? "Edit" : "New"} relationship</DialogTitle></DialogHeader>
            <div className="grid gap-3">
              <div><Label>Name *</Label><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></div>
              <div><Label>Slug (auto if empty)</Label><Input value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })} placeholder="wife" /></div>
      ...
      <div className="rounded-md border border-border bg-card divide-y divide-border">
        {items.length === 0 ? <p className="p-8 text-center text-sm text-muted-foreground">No relationships yet.</p> :
          items.map((c) => (
            <div key={c.id} className="grid grid-cols-[1fr_auto_auto] gap-2 p-3 items-center">
              <div>
                ...
                <p className="text-xs text-muted-foreground">/rel/{c.slug} · order {c.sort_order}</p>
              </div>
